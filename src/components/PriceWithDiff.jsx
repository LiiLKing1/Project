...
export function PriceWithDiff({ value, diff, className, curr = 'UZS' }) {
  ...
  return (
    <span className={cn("flex items-center gap-2", className)}>
      <NumberFlow
        ...
        format={{ style: 'currency', currency: curr, maximumFractionDigits: 0 }}
      />
      <motion.span
        className={cn(
          diff > 0 ? 'bg-emerald-500' : 'bg-red-500',
          'inline-flex items-center px-[0.4em] py-[0.1em] text-white transition-colors duration-300'
        )}
        style={{ borderRadius: 999, fontSize: '0.85em' }}
        layout={canAnimate}
        transition={{ layout: { duration: 0.9, bounce: 0, type: 'spring' } }}
      >
        <MotionArrowUp
          className="mr-0.5 size-[1em]"
          absoluteStrokeWidth
          strokeWidth={3}
          transition={{
...
export default function NumberFlowTrading() {
  const numbers = [124000, 41000, 2125000];
  const diffs = [0.056, -0.114, 0.003];
  ...
  return (
    <div>
      ...
      <button
        ...
        className="flex h-11 mt-4 w-fit items-center gap-2 rounded-full bg-slate-100 px-5 text-sm font-medium text-slate-900 transition duration-200 hover:brightness-95 active:scale-95"
      >
