...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError('Kirishda xatolik yuz berdi. Parol yoki pochtani tekshiring.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ display: 'flex', height: '100vh', alignItems: 'center', justifyContent: 'center', backgroundColor: 'var(--bg-main)' }}>
      <div style={{ background: 'var(--bg-surface)', padding: '2.5rem', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-md)', width: '100%', maxWidth: '400px' }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', marginTop: '0.5rem' }}>Tizimga kirish uchun ma'lumotlarni kiriting</p>
        </div>
        ...
        <div style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '0.875rem' }}>
          Hisobingiz yo'qmi? <Link to="/register" style={{ color: 'var(--primary)', fontWeight: '600' }}>Ro'yxatdan o'tish</Link>
        </div>
      </div>
    </div>
  );
